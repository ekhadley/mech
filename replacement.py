...
# our strongest prev token head is 4.11 with over 90% of its attn on the prev token. Second most has less than 50%.
# our induction-y heads on the repeated tokens task were 5.1, 7.2, 5.0, 6.9, 5.5, decreasing monotonically with 5.1 at about .37 iscore
# The fact that there are no standout inductiony heads lends credence to the idea that they are __literally__ just doing induction.
# probably there is some specialization in the heads, like diff types of induction, or totally different, non induction jobs on
# other prompts.
...
# Here we test our strict previous token head hook.
# It provides a very small but consisten improvement to performance on the repeated random tokens task.
tokens = gen_induction_test(16, 32)
...
# here we test our strict induction head hook.
