...
class CodeBreakingDataset(Dataset):
    ...
    def __init__(
        ...
        if mode in ["easy", "medium"]:
            ...
            self.words_3 = [word for word in words if len(word) == 3 and set(word) < set(string.ascii_lowercase)]
            words_freq = sorted(Counter(self.words_3).items(), key = lambda x: x[1], reverse=True)[:word_list_size]
            self.words_freq = [(word, freq / len(self.words_3)) for word, freq in words_freq]
            self.freq_dict = {word: freq for word, freq in self.words_freq}
            ...
            print("alksjdhhljkasdhkjlfdasdfhsjkaldfashjkladsfkhjladsfhjkl")
            self.stoks = ["".join(seq) for seq in self.str_toks]
            self.stoksr = ["".join(seq) for seq in self.str_toks_raw]
